...
export const evalSpiral2 = (data) => {
  ...
  if (LTGAPBOOL === false) {
    LTGAP = 0;
    LTGAPCR = 0;
    // TODO will have issues when returning ret
    // 'cause LTGAPCR is a local variable too
  }
  console.log(winding_conductor_ht);

  if (winding_conductor_ht == 'aluminium') {
    console.log('inside if alu');
    C3 = ALSPC3;
    C5 = MAXDEPAL;
    C6 = MAXWDAL;
  } else {
  ...
  // lt_transpose = TRANSP
  // Oops! Use this in NLCOIL1 / Spiral 1
  /*
  while (XXX == 1 || NWO != NW) {
    NWO = NW;
    if (DEP > C5) {
